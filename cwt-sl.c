...
int get_temp(modbus_t* mb, uint16_t* tab_reg) {
    int rc, i;
...
int get_humidity(modbus_t* mb, uint16_t* tab_reg) {
    int rc, i;
    rc = modbus_read_input_registers(mb, 0x0000, 0x0001, tab_reg);
...
int get_illu(modbus_t* mb, uint16_t* tab_reg) {
    int rc, i;
    rc = modbus_read_input_registers(mb, 0x0002, 0x0002, tab_reg);

    printf("Lux %d\n",  ((tab_reg[0]<<16)| tab_reg[1]));
    return rc;
}
...
int main(int argc,char** argv) {
    ...
    int set_baud_rate = -1;
    int read_all = 0;
    char* host = "192.168.3.7";
    char* serial_device = "/dev/ttyUSB0";
    int serial_baud_rate = 4800;
//    int serial_parity
    int serial_stop_bits = 1;

    while ((option = getopt(argc, argv,"h:p:m:as:d:vVfIM:B:EAD:b:S:tlH")) != -1) {
        switch (option) {
            case 'h' : host = optarg; 
                break;
...
//    rc = modbus_read_registers(mb, 0x2000, 1, tab_reg);
    ...
    if (read_temp)
	rc = get_temp(mb, tab_reg);

    if (read_hum)
        rc = get_humidity(mb, tab_reg);

    if (read_illu)
	rc = get_illu(mb, tab_reg);
exit:
    if (mb) {
        modbus_close(mb);
        modbus_free(mb);
