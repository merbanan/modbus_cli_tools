...
int get_modbus_status(modbus_t* mb, uint16_t* tab_reg) {
    ...
    rc = modbus_read_registers(mb, 0x0110, 0x0001, tab_reg);
    
    printf("Modbus id %d\n", tab_reg[0]);

    rc = modbus_read_registers(mb, 0x0111, 0x0001, tab_reg);
    baud_value   = tab_reg[0] & 0xFF;
    parity_value = tab_reg[0] >> 8; 
...
int configure_modbus_address(modbus_t* mb, uint16_t* tab_reg, int modbus_address
    ...
    tab_reg[0] = modbus_address&0xFF;
    rc = modbus_write_registers(mb, 0x0110, 0x0001, tab_reg);
...
int configure_baud_rate(modbus_t* mb, uint16_t* tab_reg, int baud_rate, int parity) {
    int rc, i;
    tab_reg[0] = ((parity&0x0F) << 8) | (baud_rate&0x0F);

    rc = modbus_write_registers(mb, 0x0111, 0x0001, tab_reg);
