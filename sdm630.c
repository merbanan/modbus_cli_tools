...
int get_all(modbus_t* mb, uint16_t* tab_reg, char* host, int slave_id) {
    ...
    // get volt L1
    rc = modbus_read_input_registers(mb, 0x0000, 0x0006, tab_reg);
    if (rc < 0) return rc;
    ...
    rc = modbus_read_input_registers(mb, 0x0004, 0x0002, tab_reg);
    if (rc < 0) return rc;
    ...
    // get energy
    rc = modbus_read_input_registers(mb, 0x0048, 0x0002, tab_reg);
    if (rc < 0) return rc;
    energy = modbus_get_float_dcba(tab_reg);

    // get power
    rc = modbus_read_input_registers(mb, 0x0034, 0x0002, tab_reg);
    if (rc < 0) return rc;
...
//    int serial_parity
    ...
    usleep(100000);
    if (modbus_tcp == 2)
	mb = modbus_new_rtu(serial_device, serial_baud_rate, 'E', 8, serial_stop_bits);
